...
consulta_discentes = """
    SELECT anosituacao, nivel,sexo, anocoleta FROM discente
...
df_discentes = pd.DataFrame(dados_discentes, columns=['anosituacao', 'nivel','sexo','anocoleta'])

# Função para consultar docentes no banco de dados

consulta_docentes = """ SELECT anocoleta FROM docente  WHERE codigoprograma = '32003013014P4' """
   ...
   if metrica_selecionada == "Proporção aluno-funcionário":
     #realiza cálculo 
      if ano_selecionado == 2020:
         for index, row in df_discentes.iterrows():
          if (row["anosituacao"] == 2016) or (row["anosituacao"] == 2017) or (row["anosituacao"] == 2018) and (row["situacao"] != "DESLIGADO"):
             ...
             contm +=1
         for index, row in df_docente.iterrows():
